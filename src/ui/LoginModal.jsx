  ...
  if (openId === name) {
    return createPortal(
      <div className="z-50 backdrop-blur-xs  fixed top-2/4 left-2/4 -translate-2/4 w-full min-h-screen flex justify-center items-center bg-stone-100/80">
        <div
          ref={ref}
          className={`relative  bg-stone-100 rounded-sm shadow-2xl px-2 py-2  h-[80vh] w-2/5  max-md:w-2/3 max-sm:w-2/3  transition-all duration-300`}
        >
          <FaXmark
            size={20}
            className="absolute top-2 right-2 cursor-pointer hover:fill-red-700"
            onClick={close}
          />
          {children}
        </div>
